...
namespace DotnetSpider.Infrastructure
{
	...
	{
		public static void FixAllRelativeHref(HtmlDocument document, Uri relativeUri)
		{
			...
			if (hrefNodes != null)
			{
				foreach (var node in hrefNodes)
				{
					...
					{
						node.Attributes["href"].Value = UriUtilities.CanonicalizeUrl(href, relativeUri);
					}
				}
			}
			...
			if (srcNodes != null)
			{
				foreach (var node in srcNodes)
				{
					...
					{
						node.Attributes["src"].Value = UriUtilities.CanonicalizeUrl(src, relativeUri);
					}
				}
			}
		}
